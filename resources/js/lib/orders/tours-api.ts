import { buildGlobalFilterParams } from '@/lib/orders/global-dashboard-filters';
import type {
...
export async function fetchTourOptions(
    signal?: AbortSignal,
): Promise<TourOption[]> {
    const response = await fetch('/api/tours/options', {
        headers: { Accept: 'application/json' },
        credentials: 'same-origin',
        signal,
    });
    ...
    if (!response.ok) {
        throw new Error('Could not load tour options.');
    }

    const body = (await response.json()) as { data?: TourOption[] };
