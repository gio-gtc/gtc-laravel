...
export function CustomSizesBlock({
    block,
    value,
    onChange,
    errors,
}: BlockRendererProps) {
    const rows = ((value as CustomSizeRow[] | undefined) ??
        []) as CustomSizeRow[];

    const emit = (next: CustomSizeRow[]) =>
        onChange(next as unknown as Parameters<typeof onChange>[0]);
                        ...
                        return (
                            ...
                            >
                                <div className="space-y-1">
                                    <Label htmlFor={`${block.key}-name-${idx}`}>
                                        Name
                                    </Label>
                                    <Input
